...
describe("ClockReducer", () => {
  ...
  it("handle set mode", () => {
    expect(
      clockReducer(undefined, set_mode(ClockMode.LONG_BREAK))
    ).toMatchObject({
  ...
  it("handle set status", () => {
    expect(
      clockReducer(undefined, set_status(ClockStatus.COUNTING_DOWN))
    ).toMatchObject({
      mode: ClockMode.POMODORO,
      playingAlarm: false,
      playingTick: false,
  ...
  it("test ticking sound", () => {
    const status = true;
    expect(
      clockReducer(undefined, set_rest_ticking_sound(status))
    ).toMatchObject({
  ...
  it("handle stop timer ", () => {
    const state = clockReducer(undefined, stop_timer());
  ...
  it("handle start timer ", () => {
    const state = clockReducer(undefined, start_timer());
    expect(state).toMatchObject({
      stoppedAt: undefined,
      baseTime: 0,
    });
