...
import type { SettingsUpdate } from "../settings/SettingsTypes";

interface ReplaceSettingsPayload {
...
export const ClockSlice = createSlice({
    ...
    reducers: {
        ...
        reset_timer: {
            reducer: (state, action: PayloadAction<ResetTimerPayload>) => {
                state.baseTime = 0;
            ...
            prepare: (): Omit<PayloadAction<ResetTimerPayload>, 'type'> => ({payload: {now: new Date().getTime()}}),
        },
        start_timer: {
            ...
            prepare: (baseTime: number = 0): Omit<PayloadAction<StartTimerPayload>, 'type'> => ({
        ...
        replaceSettings: (state, action: PayloadAction<ReplaceSettingsPayload>) => {
            const { settings } = action.payload;
            
            // 现在 settings 已经是类型安全的，不需要类型断言
            if (settings.pomodoro_duration !== undefined) {
                state.pomodoro_duration = settings.pomodoro_duration;
            }
            if (settings.short_break_duration !== undefined) {
                state.short_break_duration = settings.short_break_duration;
            }
            if (settings.long_break_duration !== undefined) {
                state.long_break_duration = settings.long_break_duration;
            }
            if (settings.ticking_sound_enabled !== undefined) {
                state.ticking_sound_enabled = settings.ticking_sound_enabled;
            }
            if (settings.rest_ticking_sound_enabled !== undefined) {
                state.rest_ticking_sound_enabled = settings.rest_ticking_sound_enabled;
            }
            if (settings.language !== undefined) {
                state.language = settings.language;
            }
            
            // 更新时间间隔，如果当前模式的时间已经变化
            switch (state.mode) {
                case ClockMode.POMODORO:
                    state.timeInterval = state.pomodoro_duration;
                    break;
