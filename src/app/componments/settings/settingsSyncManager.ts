...
import { getSettingsAPI, SettingsOperation, SettingsPayload } from './settingsAPI'
...
class SettingsAPIAdapter implements SyncAPI {
  async syncOperations(operations: SyncOperation[], lastSyncTime: string | null) {
    ...
    return await getSettingsAPI().syncSettings({
  ...
  async fetchData() {
    return await getSettingsAPI().getSettings()
  }

  setToken(token: string | null) {
    getSettingsAPI().setToken(token)
  }
}
