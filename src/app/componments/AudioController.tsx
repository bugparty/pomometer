...
class AudioController extends React.Component<Props, AudioControllerState> {
  ...
  playAlarm() {
    if (false && isSafari) {
      const audio = new Audio(this.state.alarm);
      audio.load();
      audio.play();
    } else {
      const alarm = document.getElementById("80alarm");
  ...
  render() {
    ...
    // In Next.js, static assets are served from /public directly
    const base_url = ''
